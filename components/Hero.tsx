        ...
        <div>
          ...
          {HERO_VIDEO_URL && (
            <div className="mt-6 rounded-lg overflow-hidden border border-navy/10 bg-white">
              {!play ? <button onClick={() => setPlay(true)} className="w-full text-left">{HERO_VIDEO_POSTER ? <><Image src={HERO_VIDEO_POSTER} alt="Watch how Georgia Wholesale Homes works" width={1280} height={720} className="w-full aspect-video object-cover" priority /></> : <div className="w-full aspect-video bg-navy flex items-center justify-center"><span aria-hidden="true" className="text-white text-4xl">▶</span><span className="sr-only">Play video</span></div>}</button> : <video src={HERO_VIDEO_URL} controls autoPlay playsInline preload="none" className="w-full aspect-video" />}
            </div>
          )}
        </div>
