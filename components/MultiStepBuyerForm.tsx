  ...
  async function onStep1() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (errorCode === 'invalid_phone') throw new Error('invalid_phone');
        if (errorCode === 'invalid_email') throw new Error('invalid_email');
        throw new Error('fail');
      }
      setStep(2);
    } catch (err) {
      if (err instanceof Error && err.message === 'invalid_phone') {
        setError('That phone number looks invalid. Please enter a valid mobile number.');
      } else if (err instanceof Error && err.message === 'invalid_email') {
